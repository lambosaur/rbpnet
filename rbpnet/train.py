...
def train(
    ...
    train_dataset = train_data.dataset(
        batch_size=batch_size, shuffle=shuffle, cache=cache
    )
    train_dataset = train_dataset.map(
        lambda X, Y: (
            soft_subset_dict(X, model.input_names),
            soft_subset_dict(Y, model.output_names),
        )
    )
    print("Train Dataset:")
    ...
    loss, loss_weight = create_loss(
        dataspec.tasks,
        profile_loss,
        use_bias=use_bias,
        bias_weight=profile_loss_bias_weight,
    )
    metrics = create_metrics(dataspec.tasks, profile_metrics, use_bias=use_bias)

    # compile model
    model.compile(
        loss=loss, optimizer=optimizer, loss_weights=loss_weight, metrics=metrics
    )
